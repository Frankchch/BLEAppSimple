...
fun BluetoothGattCharacteristic.printProperties(): String = mutableListOf<String>().apply {
    if (isReadable()) add("PULSAR PARA LEER")
    if (isWritable()) add("PULSAR PARA ESCRIBIR")
    if (isWritableWithoutResponse()) add("PULSAR PARA ESCRIBIR (SIN RESPUESTA)")
    if (isIndicatable()) add("INDICABLE")
    if (isNotifiable()) add("NOTIFICABLE")
    if (isEmpty()) add("VACÍO")
}.joinToString()
